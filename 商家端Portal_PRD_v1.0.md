...
1. **添加银行账户**：输入银行名称、账户类型（Checking/Savings）、账户号码、路由号码
2. **设置默认账户**：指定一个账户作为默认提现账户
3. **删除账户**：移除不再使用的银行账户
4. **账户安全**：账户号码和路由号码仅显示后4位
